        ...
        if (!(isEmpty(data.email) || isEmpty(data.password))) {
            if (validateEmail(data.email)) {
                if(size(data.password) >= 6) {
                    ...
                    setError({email: '', password: 'Se requiere una contraseña de por lo menos 6 caracteres', repeatPassword: 'Se requiere una contraseña de por lo menos 6 caracteres'})
                }
            } else {
                setError({email: 'Debe ser un Correo Electrónico', password: '', repeatPassword: ''})
            }
        } else {
